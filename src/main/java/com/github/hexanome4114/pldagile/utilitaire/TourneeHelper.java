...
public final class TourneeHelper {
     ...
     * @return La map d'itinéraire est une map dont la clé
     * est de la forme intersection1Id_intersection2Id et la valeur
     * est l'itinéraire pour aller de l'intersection1 à l'intersection2
     */
    public static Pair<Graphe, Map<String, Itineraire>> creerGrapheTSP(
            final List<Livraison> livraisons,
            final HashMap<String, Intersection> intersections,
            final Graphe grapheDijkstra
    ) {
        ...
        Map<Integer, FenetreDeLivraison> entierVersFenetreDeLivraison
                = new HashMap<>();
        Map<FenetreDeLivraison, Integer> fenetreDeLivraisonVersEntier
                = new HashMap<>();
        int compteurFdL = 0;
        // On assigne un numéro à chaque fenêtre de livraison
        for (Livraison livraison : livraisons) {
            if (fenetreDeLivraisonVersEntier.get(
                    livraison.getFenetreDeLivraison()) == null
            ) {
                compteurFdL += 1;
                entierVersFenetreDeLivraison.put(
                        compteurFdL, livraison.getFenetreDeLivraison()
                );
                fenetreDeLivraisonVersEntier.put(
                        livraison.getFenetreDeLivraison(), compteurFdL
                );
            }
        }

        // itineraireMap stocke l'itinéraire entre intersection1
        // vers intersection2, pour chaque point de livraison
        ...
        for (Livraison livraisonCourante : livraisons) {
            int numFdLCourante = fenetreDeLivraisonVersEntier.get(
                    livraisonCourante.getFenetreDeLivraison()
            );

            // On calcule la distance entre l'adresse de livraison
            // courante (sommetSource) et les autres adresse de livraisons
            Sommet sommetSource = grapheDijkstra.getSommets().get(
                    livraisonCourante.getAdresse().getId()
            );

            // Calcul des plus courts chemins entre le sommet source
            // et les autres sommets
            Graphe.calculerCheminplusCourtDepuisSource(
                    grapheDijkstra, sommetSource
            );

            // Récupération des pcc pour construire le graphe TSP
            // et les itinéraires précis correspondants.
            for (Livraison autreLivraison : livraisons) {
                if (
                        autreLivraison.getNumero()
                        != livraisonCourante.getNumero()
                ) {
                    int numAutreFdL = fenetreDeLivraisonVersEntier.get(
                            autreLivraison.getFenetreDeLivraison()
                    );
                    // On garde les arcs uniquement si les livraisons sont dans
                    // la même FdL ou si autreLivraison est dans la FdL
                    // directement subséquente ou si c'est la dernière FdL et
                    // que c'est un chemin vers le point d'arrivée (l'entrepot)
                    if (numFdLCourante == numAutreFdL
                            || numAutreFdL == (numFdLCourante + 1)
                            || (numFdLCourante == compteurFdL
                            && autreLivraison.getNumero()
                            == livraisons.get(0).getNumero())
                    ) {
                        // Sommet correspondant au point de passage dans
                        // le graphe dijktra
                        Sommet sommetLivraison
                                = grapheDijkstra.getSommets().get(
                                        autreLivraison.getAdresse().getId()
                                );

                        // Création de l'itinéraire
                        List<Intersection> intersectionsItineraire
                                = new ArrayList<>(sommetLivraison
                                .getCheminPlusCourt()
                                .size()
                        );
                        List<Sommet> plusCourtCheminCourant
                                = sommetLivraison.getCheminPlusCourt();

                        // S'il n'y a pas de chemin plus court entre les 2,
                        // on ne l'ajoute pas
                        if (!plusCourtCheminCourant.isEmpty()) {
                            for (Sommet sommet : plusCourtCheminCourant) {
                                Intersection intersectionDuPcc
                                        = intersections.get(
                                                sommet.getNom()
                                        );
                            ...
                            intersectionsItineraire.add(
                                    autreLivraison.getAdresse()
                            );
                            itineraireMap.put(
                                    livraisonCourante.getAdresse().getId()
                                    + "_"
                            ...
                            Sommet sommetSourceDansGrapheComplet
                                    = grapheTSP.getSommets().get(
                                            sommetSource.getNom()
                                    );
                            Sommet sommetDestDansGrapheComplet =
                                    grapheTSP.getSommets().get(
                                            sommetLivraison.getNom()
                                    );
                            sommetSourceDansGrapheComplet.addDestination(
                                    sommetDestDansGrapheComplet,
    ...
    public static GrapheTSP convertirGrapheVersGrapheComplet(
            final Graphe grapheEntrant
    ) {
